...
songplay_table_create = ("create table if not exists songplays (songplay_id int, start_time timestamp, user_id int, level varchar, song_id varchar, artist_id varchar, session_id varchar, location varchar, user_agent varchar)")
...
time_table_create = ("create table if not exists time (start_time numeric, hour int, day int, week int, month int, year int, weekday varchar)")
...
song_select = ("select songs.song_id, artists.artist_id from songs, artists where songs.title = %s and artists.name = %s and songs.duration = %s")
